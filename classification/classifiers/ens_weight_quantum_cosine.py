...
class EnsWeightQuantumCosineClassifier(BaseClassifier):
...
class TrainClassifier(QuantumClassifier):
    ...
    def initialize(self, x):
        ...
        qubits_num = swap_circuit_qubits+index_qubits+features_qubits+label_qubits+control_qubits

        # Create a Quantum Circuit acting on the q register
        qr = QuantumRegister(qubits_num, 'q')
        cr = ClassicalRegister(1+self.dim, 'c')
        ...
        circuit.cx(qr[ancillary_control_qubit], qr[qubits_num - self.dim - 1])
        circuit.ch(qr[ancillary_control_qubit], qr[qubits_num - self.dim - 1])
        ...
        self.label_q = swap_circuit_qubits+index_qubits+features_qubits
        ...
        #circuit.draw(output='mpl', filename='out_cosine_train.png')
        circuit.barrier()
    ...
    def classify(self, x):
        ...
        if self.execution == 'statevector':
            ...
            counts = {}
            for idx2 in range(int(2 ** (self.dim-1))):
                idx2_str = ('{0:0' + str(self.dim-1) + 'b}').format(idx2)
                counts[idx2_str + '00'] = 0
                counts[idx2_str + '01'] = 0
            for i, amplitude in enumerate(output_statevector):
               if i < len(output_statevector) // 2:
                    idx2 = i // (2**(self.label_q+1))
                    idx2 = idx2 % (2**(self.dim-1))
                    idx2_str = ('{0:0' + str(self.dim-1) + 'b}').format(idx2)
                    if (i % 2) == 0:
                        counts[idx2_str + '00'] += (np.abs(amplitude) ** 2)
                    else:
                        counts[idx2_str + '01'] += (np.abs(amplitude) ** 2)
        else:
            #local
            circuit.measure([0], [0])
        ...
        for idx2 in range(int(2 ** (self.dim-1))):
            idx2_str = ('{0:0' + str(self.dim-1) + 'b}').format(idx2)
            p0s.append(counts.get(idx2_str + '00', 0))
            p1s.append(counts.get(idx2_str + '01', 0))
        p0s = np.array(p0s)
        p1s = np.array(p1s)
        psum = p0s + p1s
        psum[psum == 0] = 1
...
class TestClassifier(BaseClassifier):
    ...
    def initialize(self, x):
        ...
        circuit.initialize(
            ctrl_amplitudes, qr[swap_circuit_qubits+index_qubits+features_qubits+label_qubits:swap_circuit_qubits+index_qubits+features_qubits+label_qubits+control_qubits-1])
        ...
        #circuit.draw(output='mpl', filename='out_cosine_test.png')
        circuit.barrier()
